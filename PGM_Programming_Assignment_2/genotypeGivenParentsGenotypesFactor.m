...
m = numAlleles * (numAlleles + 1) / 2;
genotypeFactor.card = [m, m, m];
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
for k = 1:m^3
    assignment = IndexToAssignment(k, genotypeFactor.card);
    ip1 = genotypesToAlleles(assignment(2), 1);
    jp1 = genotypesToAlleles(assignment(2), 2);
    ip2 = genotypesToAlleles(assignment(3), 1);
    jp2 = genotypesToAlleles(assignment(3), 2);
    genoProd = [allelesToGenotypes(ip1, ip2);
                allelesToGenotypes(jp1, ip2);
                allelesToGenotypes(ip1, jp2);
                allelesToGenotypes(jp1, jp2);];

    if (ismember(assignment(1), genoProd, "rows"))
        for l = 1:4
            if (assignment(1) == genoProd(l))
                genotypeFactor.val(k) += 0.25;
            endif
        end
    endif
end
