...
for m = 1:length(genotypesToAlleles)
    i = genotypesToAlleles(m, 1);
    j = genotypesToAlleles(m, 2);
    if (i == j)
        genotypeFactor.val(m) = alleleFreqs(i) ^ 2;
    else
        genotypeFactor.val(m) = 2 * alleleFreqs(i) * alleleFreqs(j);
    end
end
