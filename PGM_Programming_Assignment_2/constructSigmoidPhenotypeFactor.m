...
unit = eye(n);
weights = reshape(cell2mat(alleleWeights), n, m)';
for i = 1:2:length(phenotypeFactor.val)
    assignment = assignments(i, :);
    X = unit(assignment(2:m+1), :);
    Y = unit(assignment(m+2:end), :);
    val = computeSigmoid(sum(sum(weights .* (X + Y))));
    phenotypeFactor.val(i) = val;
    phenotypeFactor.val(i+1) = 1 - val;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%